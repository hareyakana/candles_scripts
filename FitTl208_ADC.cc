{
  ...
  ofstream fout(Form("Run%03d_Tl208/ADCRun%03d_Tl208.txt",Run,Run));
//  ofstream fout(Form("Run%03d_Tl208/NPERun%03d_Tl208.txt",Run,Run));
...
//    TString inputfile = Form("Run%03d_histogram/NPEhistogram_Run%03d-%03d.root",Run,Run,SubRun);
    ...
    if (data -> GetNkeys() != 0){
      ...
      histo -> GetXaxis() -> SetRangeUser(2200,3000);
...
//      histo -> GetXaxis() -> SetTitle(Form("Energy[%d] [keV]",status));
      ...
      // 208Tl 2.615MeV
      TF1* f1 = new TF1("f1","gaus",2535,2830); // par 0,1,
      histo -> Fit("f1","L","",2535,2830);
      
      TF1* f2 = new TF1("f2","expo",2450,1900); 
      histo -> Fit("f2","+","",2450,1900);
    
      TF1* func = new TF1("func","gaus(0)+expo(3)",2420,2900);
      Double_t par[5];
      f1->GetParameters(&par[0]);
      f2->GetParameters(&par[3]);
      func -> SetParameters(par);
      histo -> Fit(func,"","",2420,2900);
      ...
      TString pathEnergyHisto = Form("Run%03d_Tl208/ADCRun%03d-%03d.pdf",Run,Run,SubRun);
//      TString pathEnergyHisto = Form("Run%03d_Tl208/NPERun%03d-%03d.pdf",Run,Run,SubRun);
